...
let
  cfg = config.rv32ima.services.ssh-agent;
in
{
  imports =
    map
      (
        shell: lib.mkRemovedOptionModule [ "rv32ima" "services" "ssh-agent" "enable${shell}Integration" ] ""
      )
      [
        "Bash"
        "Zsh"
  ...
  options.rv32ima.services.ssh-agent = {
